function varargout = MMF7(Operation,Global,input)
% <problem> <MMF>
...
%---------------------------------------------------------------------------
    ...
    switch Operation
        ...
        case 'value'
            ...
            PopObj(:,2) = 1 - sqrt(PopObj(:,1)) + (PopDec(:,2)-(0.3.*(PopObj(:,1).^2).*cos(24.*pi.*PopObj(:,1)+4.*pi)+0.6.*PopObj(:,1)).*sin(6.*pi.*PopObj(:,1)+pi)).^2;
            PopCon = [];
